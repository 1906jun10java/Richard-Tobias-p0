...
public class CarDAOImp implements CarDAO {
	...
	public static void displayCurrentLot(){
        try {
            Scanners.cdao.getCarList();
            Scanners.idao.getInvoiceList();
	...
	/*
	 * public static void displayCurrentLot(){
	 * System.out.println("Getting Cars In Lot"); boolean[] checker = new
	 * boolean[InvoiceDAOImp.invoiceList.size()]; for(int i = 0; i < carList.size();
	 * i++) { for(int j = 0; j <InvoiceDAOImp.invoiceList.size(); j++) {
	 * if(carList.get(i).getCar_id() ==
	 * InvoiceDAOImp.invoiceList.get(j).getCar_id()) { //System.out.println("Car " +
	 * j + " has been purchased."); checker[j] = false;
	 * 
	 * }else { //System.out.println("Car " + j + " has not been purchased.");
	 * checker[j] = true; } }
	 * 
	 * int exists = 0; for(int k = 0; k < checker.length; k++) { if (checker[k] ==
	 * false) { exists++; } } if(exists > 0) { continue; }else {
	 * currentLot.add(carList.get(i)); System.out.println(carList.get(i)); } }
	 * 
	 * }
	 */
